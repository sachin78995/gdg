...
const DEFAULT_GLOW_COLOR = '66, 133, 244'; // GDG blue-ish
const MOBILE_BREAKPOINT = 768;

const createParticleElement = (x, y, glowColor) => {
...
/**
 * cards: [{ id?, title, description, label, image?, color?, chip?, onClick? }]
 */
const MagicBento = ({
  cards,
  textAutoHide = true,
    ...
    <div className="bento-section">
      ...
      <div className="card-grid" ref={gridRef}>
        {safeCards.map((card, index) => {
          ...
          } ${enableBorderGlow ? 'magic-bento-card--border-glow' : ''} ${card.className || ''}`.trim();
          ...
          const avatarInitials = (card.title || '')
            .split(' ')
            .filter(Boolean)
            .slice(0, 2)
          ...
          const content = card.render ? (
            <div className="magic-bento-card__custom">{card.render({ card, index })}</div>
          ) : (
            <>
              <div className="magic-bento-card__header">
                <div className="magic-bento-card__header-left">
                  <div className="magic-bento-card__avatar" aria-hidden="true">
                    {card.image ? (
                      <img src={card.image} alt="" onError={(e) => (e.currentTarget.style.display = 'none')} />
                    ) : (
                      <div className="magic-bento-card__avatar-fallback">{avatarInitials}</div>
                    )}
                  </div>
              ...
              <div className="magic-bento-card__footer">
                <span className="magic-bento-card__chip">{card.chip || 'View profile'}</span>
                <span className="magic-bento-card__arrow" aria-hidden="true">
                  →
                </span>
              </div>
          ...
          const handleClick = () => card.onClick?.(card, index);
          ...
          if (enableStars) {
            return (
              <ParticleCard
                ...
                onClick={handleClick}
                aria-label={card.ariaLabel || `${card.title}${card.description ? ` — ${card.description}` : ''}`}
              >
                {content}
              </ParticleCard>
            );
